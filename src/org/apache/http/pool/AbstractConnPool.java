...
ConnPoolControl<T> {
    ...
    private E getPoolEntryBlocking(T route, Object state, long timeout, TimeUnit tunit, PoolEntryFuture<E> future) throws IOException, InterruptedException, TimeoutException {
        ...
        this.lock.lock();
        try {
            RouteSpecificPool<T, C, PoolEntry<T, C>> pool = this.getPool(route);
            ...
            while (entry == null) {
                ...
                int totalUsed;
                Asserts.check(!this.isShutDown, "Connection pool shut down");
                while ((entry = (PoolEntry<T, C>)pool.getFree(state)) != null && (entry.isClosed() || entry.isExpired(System.currentTimeMillis()))) {
                    entry.close();
