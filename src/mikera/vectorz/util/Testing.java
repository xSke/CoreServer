...
public class Testing {
    public static AVector createTestVector(int length) {
        return Testing.createTestVector(length, new Random());
    }
    ...
    public static AVector createTestVector(int length, Random r) {
        ...
        if (r.nextInt(100) < 50) {
            ...
            return Testing.createTestVector(mid, r).join(Testing.createTestVector(length - mid, r));
        }
    ...
    public static boolean validateFullyMutable(INDArray m) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (t == (x = 10.0 + Rand.nextDouble() * 1000.0)) {
                x += 1.0;
            }
            v.set(i, x);
            if (x != v.get(i)) {
