...
Serializable {
    ...
    private int externalAwaitDone() {
        ...
        if (s >= 0) {
            ...
            if (s >= 0 && (s = this.status) >= 0) {
                ...
                do {
                    if (!U.compareAndSwapInt(this, STATUS, s, s | 65536)) continue;
                    ForkJoinTask forkJoinTask = this;
                    synchronized (forkJoinTask) {
                        if (this.status >= 0) {
                            try {
    ...
    public static void invokeAll(ForkJoinTask<?> t1, ForkJoinTask<?> t2) {
        int s2;
        t2.fork();
        int s1 = ForkJoinTask.super.doInvoke() & -268435456;
        if (s1 != -268435456) {
            ForkJoinTask.super.reportException(s1);
        }
        if ((s2 = ForkJoinTask.super.doJoin() & -268435456) != -268435456) {
            ForkJoinTask.super.reportException(s2);
        }
    }

    public static /* varargs */ void invokeAll(ForkJoinTask<?> ... tasks) {
        ForkJoinTask t;
        int i;
        int last;
        Throwable ex = null;
        for (i = last = tasks.length - 1; i >= 0; --i) {
            ...
            if (ForkJoinTask.super.doInvoke() >= -268435456 || ex != null) continue;
            ex = t.getException();
        }
        for (i = 1; i <= last; ++i) {
            t = tasks[i];
            ...
            if (ForkJoinTask.super.doJoin() >= -268435456) continue;
            ex = t.getException();
        }
        if (ex != null) {
        ...
        if (s >= 0 && ns > 0L) {
            ...
            try {
                while ((s = this.status) >= 0) {
                    ...
                    if (ms > 0L && U.compareAndSwapInt(this, STATUS, s, s | 65536)) {
                        ForkJoinTask forkJoinTask = this;
                        synchronized (forkJoinTask) {
                            if (this.status >= 0) {
                                try {
    ...
    public final short setForkJoinTaskTag(short tag) {
        int s;
        while (!U.compareAndSwapInt(this, STATUS, s = this.status, s & -65536 | tag & 65535)) {
        }
        return (short)s;
    }
    ...
    static {
        ...
        exceptionTableRefQueue = new ReferenceQueue<T>();
