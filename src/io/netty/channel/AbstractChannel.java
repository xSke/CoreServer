    ...
    implements Channel.Unsafe {
                    ...
                    public void run() {
                        try {}
                            ...
                            AbstractUnsafe.this.invokeLater(new OneTimeTask(){
                                ...
                                @Override
                                public void run() {
                                    buffer.failFlushed(AbstractChannel.CLOSED_CHANNEL_EXCEPTION, false);
                                    buffer.close(AbstractChannel.CLOSED_CHANNEL_EXCEPTION);
        ...
        protected void flush0() {
            ...
            if (!AbstractChannel.this.isActive()) {
                try {
                    if (AbstractChannel.this.isOpen()) {
                        outboundBuffer.failFlushed(AbstractChannel.NOT_YET_CONNECTED_EXCEPTION, true);
                    } else {
                        outboundBuffer.failFlushed(AbstractChannel.CLOSED_CHANNEL_EXCEPTION, false);
                    }
                }
                finally {
                    this.inFlush0 = false;
            ...
            try {
                ...
                boolean close = t instanceof IOException && AbstractChannel.this.config().isAutoClose();
        ...
        protected final Throwable annotateConnectException(Throwable cause, SocketAddress remoteAddress) {
            if (cause instanceof ConnectException) {
                ConnectException newT = new ConnectException(cause.getMessage() + ": " + remoteAddress);
                newT.setStackTrace(cause.getStackTrace());
                cause = newT;
            } else if (cause instanceof NoRouteToHostException) {
                NoRouteToHostException newT = new NoRouteToHostException(cause.getMessage() + ": " + remoteAddress);
                newT.setStackTrace(cause.getStackTrace());
                cause = newT;
            } else if (cause instanceof SocketException) {
                SocketException newT = new SocketException(cause.getMessage() + ": " + remoteAddress);
                newT.setStackTrace(cause.getStackTrace());
                cause = newT;
            }
            return cause;
