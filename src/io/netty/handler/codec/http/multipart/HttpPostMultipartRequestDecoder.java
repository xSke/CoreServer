...
implements InterfaceHttpPostRequestDecoder {
            ...
            if (contents[0].equalsIgnoreCase("Content-Disposition")) {
                ...
                for (int i = 2; i < contents.length; ++i) {
                    ...
                    try {
                        ...
                        value = "filename".equals(name) ? value.substring(1, value.length() - 1) : HttpPostMultipartRequestDecoder.cleanString(value);
            ...
            if (contents[0].equalsIgnoreCase("Content-Type")) {
                ...
                for (int i = 1; i < contents.length; ++i) {
                    ...
                    if (contents[i].toLowerCase().startsWith("charset")) {
                        ...
                        try {
                            attribute2 = this.factory.createAttribute(this.request, "charset", HttpPostMultipartRequestDecoder.cleanString(values));
                        }
                        catch (NullPointerException e) {
                            throw new HttpPostRequestDecoder.ErrorDataDecoderException(e);
                        }
    ...
    protected InterfaceHttpData getFileUpload(String delimiter) {
        ...
        if (this.currentFileUpload == null) {
            ...
            try {
                this.currentFileUpload = this.factory.createFileUpload(this.request, HttpPostMultipartRequestDecoder.cleanString(nameAttribute.getValue()), HttpPostMultipartRequestDecoder.cleanString(filenameAttribute.getValue()), contentTypeAttribute.getValue(), mechanism.value(), localCharset, size);
            }
            catch (NullPointerException e) {
                throw new HttpPostRequestDecoder.ErrorDataDecoderException(e);
            }
    ...
    private static String[] splitMultipartHeader(String sb) {
        ...
        String[] values = svalue.indexOf(59) >= 0 ? HttpPostMultipartRequestDecoder.splitMultipartHeaderValues(svalue) : StringUtil.split(svalue, ',');
