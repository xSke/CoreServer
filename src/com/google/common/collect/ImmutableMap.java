...
Serializable {
    ...
    static void checkNoConflict(boolean safe, String conflictDescription, Map.Entry<?, ?> entry1, Map.Entry<?, ?> entry2) {
        if (!safe) {
            String string = String.valueOf(String.valueOf(conflictDescription));
            String string2 = String.valueOf(String.valueOf(entry1));
            String string3 = String.valueOf(String.valueOf(entry2));
            throw new IllegalArgumentException(new StringBuilder(34 + string.length() + string2.length() + string3.length()).append("Multiple entries with same ").append(string).append(": ").append(string2).append(" and ").append(string3).toString());
        }
    }
    ...
    public ImmutableCollection<V> values() {
        ...
        ImmutableCollection<V> immutableCollection = result == null ? (this.values = new ImmutableMapValues<K, V>(this)) : result;
    ...
    public ImmutableSetMultimap<K, V> asMultimap() {
        ...
        ImmutableSetMultimap<K, V> immutableSetMultimap = result == null ? (this.multimapView = this.createMultimapView()) : result;
        ...
        ImmutableSet<Map.Entry<K, ImmutableSet<V>>> createEntrySet() {
            return new ImmutableMapEntrySet<K, ImmutableSet<V>>(){
                ...
                public UnmodifiableIterator<Map.Entry<K, ImmutableSet<V>>> iterator() {
                    final Iterator backingIterator = MapViewOfValuesAsSingletonSets.this.delegate.entrySet().iterator();
