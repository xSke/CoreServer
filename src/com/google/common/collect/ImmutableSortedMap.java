...
implements NavigableMap<K, V> {
    ...
    static <K, V> ImmutableSortedMap<K, V> fromSortedEntries(Comparator<? super K> comparator, int size, Map.Entry<K, V>[] entries) {
        ...
        return new RegularImmutableSortedMap(new RegularImmutableSortedSet<K>((ImmutableList<? super K>)keyBuilder.build(), comparator), valueBuilder.build());
    }
    ...
    static /* varargs */ <K, V> ImmutableSortedMap<K, V> fromEntries(Comparator<? super K> comparator, boolean sameComparator, int size, Map.Entry<K, V> ... entries) {
        for (int i = 0; i < size; ++i) {
            Map.Entry<K, V> entry = entries[i];
