...
extends ImmutableTable<R, C, V> {
    ...
    private static final <R, C, V> RegularImmutableTable<R, C, V> forCellsInternal(Iterable<Table.Cell<R, C, V>> cells, @Nullable Comparator<? super R> rowComparator, @Nullable Comparator<? super C> columnComparator) {
        void columnSpace22;
        ImmutableSet.Builder<E> rowSpaceBuilder = ImmutableSet.builder();
        ImmutableSet.Builder<E> columnSpaceBuilder = ImmutableSet.builder();
        ImmutableList<Table.Cell<R, C, V>> cellList = ImmutableList.copyOf(cells);
        for (Table.Cell cell : cellList) {
            rowSpaceBuilder.add(cell.getRowKey());
            columnSpaceBuilder.add(cell.getColumnKey());
        }
        ImmutableSet<E> rowSpace = rowSpaceBuilder.build();
        if (rowComparator != null) {
            ArrayList<E> rowList = Lists.newArrayList(rowSpace);
            Collections.sort(rowList, rowComparator);
            rowSpace = ImmutableSet.copyOf(rowList);
        }
        ImmutableCollection columnSpace22 = columnSpaceBuilder.build();
        if (columnComparator != null) {
            ArrayList<E> columnList = Lists.newArrayList(columnSpace22);
            Collections.sort(columnList, columnComparator);
            ImmutableSet<E> columnSpace22 = ImmutableSet.copyOf(columnList);
        }
        return (long)cellList.size() > (long)rowSpace.size() * (long)columnSpace22.size() / 2L ? new DenseImmutableTable<R, C, V>(cellList, (ImmutableSet<R>)rowSpace, (ImmutableSet<C>)columnSpace22) : new SparseImmutableTable<R, C, V>(cellList, (ImmutableSet<R>)rowSpace, (ImmutableSet<C>)columnSpace22);
    }
