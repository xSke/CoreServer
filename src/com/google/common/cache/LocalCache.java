...
implements ConcurrentMap<K, V> {
                ...
                try {
                    ...
                    for (Object key : keysToLoad) {
                        ...
                        if (value == null) {
                            ...
                            throw new CacheLoader.InvalidCacheLoadException(new StringBuilder(37 + string.length()).append("loadAll failed to return a value for ").append(string).toString());
                        }
                        result.put(key, value);
                    }
                }
    ...
    Map<K, V> loadAll(Set<? extends K> keys, CacheLoader<? super K, V> loader) throws ExecutionException {
        ...
        if (nullsPresent) {
            this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
            String i$ = String.valueOf(String.valueOf(loader));
            throw new CacheLoader.InvalidCacheLoadException(new StringBuilder(42 + i$.length()).append(i$).append(" returned null keys or values from loadAll").toString());
        }
